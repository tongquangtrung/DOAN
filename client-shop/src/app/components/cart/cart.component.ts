  ...
  update(id: number, quantity: number) {
    if (quantity < 1) {
      ...
      this.cartService.getOneDetail(id).subscribe(data => {
        ...
        // nếu số lượng mua lớn hơn số lượng trong kho thì thông báo lỗi
        if (quantity > this.cartDetail.product.quantity) {
          this.toastr.error('Số lượng vượt quá số lượng có trong kho', 'Lỗi');

          // nếu số lượng mua nhỏ hơn hoặc bằng số lượng trong kho thì cập nhật số lượng
