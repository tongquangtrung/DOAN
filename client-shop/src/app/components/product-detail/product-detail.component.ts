...
export class ProductDetailComponent implements OnInit {
  ...
  // Trong hàm addCart, bổ sung kiểm tra số lượng
  addCart(productId: number, price: number) {
    ...
    // Kiểm tra số lượng trực tiếp trên object product hiện tại (nếu là sản phẩm đang xem)
    if (this.product && this.product.productId === productId && this.product.quantity <= 0) {
      this.toastr.warning('Sản phẩm này hiện đang hết hàng!', 'Hệ thống');
      return;
    }
    ...
    this.cartService.getCart(email).subscribe(data => {
      ...
      this.cartService.postDetail(this.cartDetail).subscribe({
        next: (res) => {
          ...
          this.cartService.getAllDetail(this.cart.cartId).subscribe(details => {
            this.cartDetails = details as CartDetail[];
            this.cartService.setLength(this.cartDetails.length);
          });
        },
        error: (err) => {
          this.toastr.error('Không thể thêm vào giỏ hàng, vui lòng thử lại!', 'Hệ thống');
        }
      });
    });
  }
